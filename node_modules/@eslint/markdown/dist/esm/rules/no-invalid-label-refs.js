...
function findInvalidLabelReferences(node, sourceCode) {
    ...
    while (startIndex < nodeText.length) {
        ...
        const { lineOffset: startLineOffset, columnOffset: startColumnOffset } = findOffsets(nodeText, nodeMatchIndex + 1);
        // find location of [ in the document text
        const { lineOffset: endLineOffset, columnOffset: endColumnOffset } = findOffsets(nodeText, nodeMatchIndex + match[0].length);
